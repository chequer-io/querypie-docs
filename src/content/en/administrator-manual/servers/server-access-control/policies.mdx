...
QueryPie server access policies (Policy) are functions for managing and applying access policies for each server managed by the organization. You can set detailed policies such as when to allow access and which commands to prohibit for each server or server group, so that users or groups can access only the minimum necessary servers according to their business purposes.
