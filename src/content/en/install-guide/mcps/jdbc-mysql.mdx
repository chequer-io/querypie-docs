...
Connect to MySQL database to retrieve and manage data.
...
### Input Parameters
- **Host** (required): MySQL server host
...
### execute_sql
Executes SQL queries on MySQL database and returns results.
...
## MySQL Connection Setup Method
1. Prepare a user account that can access the MySQL server.
2. Grant necessary database permissions.
3. Configure network access permissions.
4. Select MySQL integration in QueryPie AI.
5. Enter connection information to complete the setup.

## Notes
- Keep MySQL user account passwords secure.
