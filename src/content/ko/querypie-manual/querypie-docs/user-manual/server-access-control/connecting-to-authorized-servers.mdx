...
1. Servers 메뉴로 첫 접속시 표시되는 Role 선택 화면에서 접속을 위해 사용할 Role을 선택합니다.Role에 따라 서버 접근 권한이 다르며, 부여된 Role이 없는 경우 Default Role이 자동으로 선택됩니다. Default Role은 Workflow를 통해 받은 권한이나 관리자가 Role 없이 직접 부여한 접근 권한을 의미합니다.
...
1. Role 선택 후에는 좌측의 패널에서 현재 사용자가 접근 가능한 서버 목록을 확인할 수 있습니다Sort by Server Group : . 서버 목록은 서버 그룹이 우선 정렬되며 하단에 개별 서버가 정렬됩니다. 만약 정렬 방식을 서버 그룹 없이 서버로만 표시하길 원한다면 좌측 상단에 있는 Sort by Server Group 버튼을 Off 상태로 변경하면 됩니다. Time-limited items only : 시간단위로 권한을 부여받은 서버만 리스트에서 표시합니다.
...
2. 서버 목록에서 서버를 선택하면 우측 페이지에서 서버 및 계정 정보를 확인할 수 있습니다. Server OS : 해당 서버의 OS를 표시됩니다 Host : 해당 서버의 호스트가 표시됩니다. Account : 권한이 있는 계정 목록입니다. 계정 뒤의 괄호 안에는 서버 및 계정이 속한 서버 그룹이 표시됩니다.View Detailed Policy Application : 선택한 계정에 적용된 정책을 표시합니다.Access Time : 접속 가능 시간을 표시합니다.Weekday Access Allowed : 접속 가능 요일을 표시합니다.Restrict Commands (SSH) : SSH 접속시 차단 명령어를 표시합니다.Restrict Functions (SFTP) : SFTP 접속시 차단 기능을 표시합니다.Password(또는 SSH Key) : 선택한 계정의 인증 방식이 Password 일 경우 Password를 입력할 수 있는 필드가 표시됩니다. 만약 자동 로그인 설정이 되어있다면 Password 항목이 표시되지 않으며, 관리자가 설정한 패스워드 또는 SSH Key로 로그인을 하게 됩니다. Status : 선택한 계정의 상태가 표시됩니다. Protocol : SSH, SFTP 또는 RDP 를 선택하여 접속할 수 있습니다.
  1. Server OS : 해당 서버의 OS를 표시됩니다
  2. Host : 해당 서버의 호스트가 표시됩니다.
  3. Account : 권한이 있는 계정 목록입니다. 계정 뒤의 괄호 안에는 서버 및 계정이 속한 서버 그룹이 표시됩니다.
  4. View Detailed Policy Application : 선택한 계정에 적용된 정책을 표시합니다.Access Time : 접속 가능 시간을 표시합니다.Weekday Access Allowed : 접속 가능 요일을 표시합니다.Restrict Commands (SSH) : SSH 접속시 차단 명령어를 표시합니다.Restrict Functions (SFTP) : SFTP 접속시 차단 기능을 표시합니다.
...
<div class="alert alert-info">
  <i class="fas fa-info-circle"></i> <strong>Info:</strong> **Q. 접속에 실패했다는 에러가 뜹니다. 왜 그런 건가요? ** 
...
<div class="alert alert-info">
  <i class="fas fa-info-circle"></i> <strong>Info:</strong> **Q. RDP를 통한 Windows Server 접속은 어떻게 하나요?** 
...
<div class="alert alert-info">
  <i class="fas fa-info-circle"></i> <strong>Info:</strong> **Q. VNC를 통한 macOS 접속은 어떻게 하나요?** 
