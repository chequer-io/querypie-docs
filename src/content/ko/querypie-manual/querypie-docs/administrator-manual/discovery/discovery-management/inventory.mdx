...
<p align="center">
<div>[image-20240714-055033.png]()</div>
...
<div class="alert alert-info">
  <i class="fas fa-info-circle"></i> <strong>Info:</strong> **QueryPie에서 사용하는 태그의 종류** 
[ol]
[li]
QueryPie Tag (System Tag) : 시스템에서 자동 부여된 태그로 기본적으로 사용자가 수정하거나 추가할 수 없습니다. 

[li]
Provider Tag (Cloud sync tag) : 클라우드에서 동기화한 태그입니다. 각 클라우드는 관리되고 있는 태그가 있을 수 있습니다. 이 태그는 클라우드에서 가져왔기 때문에 QueryPie에서 수정, 추가, 삭제 할 수 없습니다. 

[li]
Custom Tag : 사용자가 QueryPie 내에서 자유롭게 부여할 수 있는 태그입니다. 

[li]
Resource Tag : 각 커넥션에는 클라우드에서 가져온 Provider tag와 사용자가 부여한 Custom tag 두가지가 존재할 수 있습니다. 이를 Resource Tag라고 부릅니다. 
...
## Inventory 목록
<p align="center">
<div>[image-20240730-041253.png]()</div>
</p>
...
<p align="center">
<div>[image-20240730-043435.png]()</div>
</p>

Resource Tags에는 커넥션에 부여된 태그(Provider tag 및 Custom Tag)가 표시되고 + Add Tags을 클릭하여 태그관리 수행할 수 있습니다.

## 커넥션 하위의 데이터 경로
<p align="center">
<div>[image-20241030-013932.png]()</div>
*커넥션 하위의 data path 목록과 지정된 민감정보 태그*
</p>

스캔이 최소 1회 성공적으로 완료된 커넥션은 대상 데이터베이스의 컬럼까지의 경로를 보여줍니다. 경로는 아래와 같은 구조로 구성되어 있습니다.

만약 해당 경로에 스캔 결과의 검토로 확정된 민감정보를 포함하고 있는 컬럼이 존재하다면 민감정보 태그와 유형, 관련된 컴플라이언스 정보를 같이 확인할 수 있습니다.
...
2. Sensitive Items 항목의 Add New Item 버튼을 클릭합니다.Sensitive Items 항목의 Add New Item 버튼  민감정보에 맞는 태그를 선택하고 OK 버튼을 누릅니다.Add New Item 버튼을 클릭하면 출력되는 태그 입력 대화창False Negative 처리된 결과를 확인합니다.수동 입력된 민감정보 태그 확인
  1. 민감정보에 맞는 태그를 선택하고 OK 버튼을 누릅니다.Add New Item 버튼을 클릭하면 출력되는 태그 입력 대화창
  2. False Negative 처리된 결과를 확인합니다.수동 입력된 민감정보 태그 확인

## 민감정보 태그 제거
스캔결과에 대한 검토 결과로 부여된 민감정보 태그와 수동으로 부여한 민감정보 태그 모두 Inventory에서 제거할 수 있습니다.

<p align="center">
<div>[image-20241030-021053.png]()</div>
*기존에 지정된 민감 정보 태그 제거*
</p>

1. 제거할 민감 정보 태그를 선택합니다.
2. Remove 버튼을 누릅니다.
3. 확인 대화창에서 Remove 버튼을 누릅니다.제거 전 확인 대화창

## 컬럼에 태그 부여
각 데이터 경로에 있는 Add Custom Tags 버튼을 사용하여 특정 컬럼에 사용자 정의 태그를 부여할 수 있습니다.

<p align="center">
<div>[image-20241030-033748.png]()</div>
*Add Custom Tag*
</p>
...
<p align="center">
<div>[image-20241030-034024.png]()</div>
*태그 입력*
</p>

1. 입력된 태그를 확인합니다. 태그 추가가 완료된 상태태그 옆의 X를 누르면 부여한 커스텀 태그가 제거됩니다.

## 제약사항
* 민감 정보 탐색이 지원되는 대상: MySQL, MariaDB, Oracle, PostgreSQL, MS SQLserver, Azure SQL탐색이 수행되어야 DB의 경로를 수집할 수 있기 때문에 지원하지 않는 DB의 data path는 표시되지 않습니다.
