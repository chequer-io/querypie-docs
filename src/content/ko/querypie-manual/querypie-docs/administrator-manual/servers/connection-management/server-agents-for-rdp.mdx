...
<p align="center">
<div>[image-20240902-050202.png]()</div>
...
4. 테이블에서 이하의 컬럼 정보를 제공합니다:Name : 서버 에이전트 명(Windows Server의 Computer name)Host : 서버의 IPLinked Server : 서버 에이전트와 연결된 서버Status : 서버 에이전트의 상태Offline : 서버 에이전트 오프라인 또는 통신 불가능한 상태Online : 서버 에이전트 온라인이며, RDP 접속이 가능한 상태Connection Fail : 서버 에이전트에서 응답은 오지만, 패킷을 전송할 수 없는 상태이며, Connection Fail을 클릭하면 대상 windows server로 연결할 IP 주소를 수동으로 입력 가능Linked At : 서버 에이전트와 최초로 통신된 시간Updated At : 서버 에이전트와 가장 최근 통신한 시간
  1. Name : 서버 에이전트 명(Windows Server의 Computer name)
  2. Host : 서버의 IP
  3. Linked Server : 서버 에이전트와 연결된 서버
  4. Status : 서버 에이전트의 상태Offline : 서버 에이전트 오프라인 또는 통신 불가능한 상태Online : 서버 에이전트 온라인이며, RDP 접속이 가능한 상태Connection Fail : 서버 에이전트에서 응답은 오지만, 패킷을 전송할 수 없는 상태이며, Connection Fail을 클릭하면 대상 windows server로 연결할 IP 주소를 수동으로 입력 가능
  ...
  5. Linked At : 서버 에이전트와 최초로 통신된 시간
  6. Updated At : 서버 에이전트와 가장 최근 통신한 시간
...
3. Server Agent 설치 및 제거하기 가이드를 통해 Server Agent의 설치 방법을 확인할 수 있습니다.

<div class="alert alert-info">
  <i class="fas fa-info-circle"></i> <strong>Note:</strong> RDP Server Agent와 연결된 Windows Server의 Port 및 IP는 변경할 수 없습니다. 
...
<p align="center">
<div>[image-20240731-024502.png]()</div>
</p>

1. Administrator > Servers > Connection Management > Server Agents for RDP 메뉴로 이동합니다.
2. Connection Failed 항목을 클릭합니다.
3. 나타나는 팝업의 Host에 Windows Server에 접속이 가능한 IP를 입력 후 Save버튼을 클릭합니다.

## Server Agents 삭제하기
<p align="center">
<div>[image-20240728-154821.png]()</div>
</p>
...
<div class="alert alert-info">
  <i class="fas fa-info-circle"></i> <strong>Note:</strong> QueryPie에서는 Windows Server에 설치된 Server Agents를 먼저 삭제한 후, QueryPie의 Server Agents for RDP 메뉴에서 Server Agents 삭제해야 합니다. 
