...
<p align="center">
<div>[image-20250512-110402.png]()</div>
*Databases &gt; Monitoring &gt; Running Queries*
</p>

1. Databases > Monitoring > Running Queries 메뉴로 이동합니다.
2. 당일 기준으로 내림차순으로 로그가 조회됩니다.
3. 테이블 좌측 상단의 필터 버튼을 클릭하여 AND/OR 조건으로 이하의 필터링이 가능합니다.SQL Type : SQL 구문 유형Action At : 쿼리 실행 일시 범위Executed From : 쿼리 실행 주체Web Editor : 쿼리파이 웹 에디터에서 실행Proxy : 쿼리파이 프록시를 통한 실행SQL Request : SQL Request 워크플로를 통한 실행SQL Job : SQL Job에 의한 실행SQL Export Request : SQL Export Request 워크플로를 통한 실행
  1. SQL Type : SQL 구문 유형
  2. Action At : 쿼리 실행 일시 범위
  3. Executed From : 쿼리 실행 주체Web Editor : 쿼리파이 웹 에디터에서 실행Proxy : 쿼리파이 프록시를 통한 실행SQL Request : SQL Request 워크플로를 통한 실행SQL Job : SQL Job에 의한 실행SQL Export Request : SQL Export Request 워크플로를 통한 실행
...
<p align="center">
<div>[image-20250512-110509.png]()</div>
*Databases &gt; Monitoring &gt; Running Queries &gt; Running Query Details*
</p>

* 우측 드로워에는 이하의 정보를 노출합니다:Name : 대상 사용자 이름Action Type : 액션 유형Privilege Name : 사용자 접속 권한Privilege Type : 사용자 접속 권한 유형SQL Type : SQL 구문 유형Executed From : 쿼리 실행 주체Connection Name : 대상 DB 커넥션명Replication Type : DB Replication 유형Client Name : 이용 클라이언트명 (DataGrip 등)Client IP : 사용자 클라이언트 IP 주소Database Type : 대상 데이터베이스 유형DB Host : 접속한 DB 호스트DB Name : DB명DB User : DB 사용자 IDTable(s) : 호출 테이블명Query : 실행 쿼리문
  1. Name : 대상 사용자 이름
  2. Action Type : 액션 유형
...
<div class="alert alert-info">
  <i class="fas fa-info-circle"></i> <strong>Info:</strong> Proxy를 통해 수행한 쿼리 정보를 조회할 수는 있으나 중지할 수 없습니다. 만약 중지를 원한다면, Proxy Management에서 세션을 중지해야 합니다. 

</div>

<p align="center">
<div>[image-20250512-111342.png]()</div>
*쿼리 중지하기*
</p>
