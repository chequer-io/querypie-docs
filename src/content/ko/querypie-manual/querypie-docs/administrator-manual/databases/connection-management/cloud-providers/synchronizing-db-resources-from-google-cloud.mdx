...
<p align="center">
<div>[image-20241217-123654.png]()</div>
...
6. 리소스를 동기화하기 위해 필요한 Credential 정보를 입력합니다.Synchronize 버튼 클릭시 GCP의 Service Account를 입력하는 수동 동기화 방식을 기본 제공합니다. QueryPie 10.2.2 부터 “Save Credential for Synchronization” 옵션을 제공하여 Credential type로 Service Account를 사용 할 때도 스케줄을 통한 동기화가 가능하도록 개선되었습니다.
  * Synchronize 버튼 클릭시 GCP의 Service Account를 입력하는 수동 동기화 방식을 기본 제공합니다. QueryPie 10.2.2 부터 “Save Credential for Synchronization” 옵션을 제공하여 Credential type로 Service Account를 사용 할 때도 스케줄을 통한 동기화가 가능하도록 개선되었습니다.
...
<p align="center">
<div>[image-20241220-095231.png]()</div>
...
5. Cloud Provider 를 한번 등록하고 나면 Provider 기본 정보 및 인증정보는 변경할 수 없습니다.Name: 변경 가능Cloud Provider: 변경 불가Project ID: 변경 불가Credential: 변경 불가“Save Credential for Synchronization” : 비활성화 상태는 활성화로 변경할 수 있으나 활성화 상태를 비활성화로 전환 불가Database Type Filter: 변경 가능Replication Frequency: 변경 가능
  1. Name: 변경 가능
...
2. 표시되는 Service Account JSON 항목에 JSON 키 값을 입력 후 Ok 버튼을 클릭합니다.Service Account 는 Google Cloud 콘솔 > IAM 및 관리자 > 서비스 계정 메뉴에서 생성 후 JSON 키를 발급하여 사용하실 수 있습니다.
...
<div class="alert alert-info">
  <i class="fas fa-info-circle"></i> <strong>Info:</strong> **GCP 에서 Service Account 를 생성하실 때 다음의 권한이 필요합니다. ** 
[ul]
[li]
