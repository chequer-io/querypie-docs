...
## Prerequisites
* 이하의 쿼리파이 관리자 권한 Role에서 해당 기능을 활성화 할 수 있습니다: OwnerSystem Admin
  * Owner
  * System Admin
...
5. 팝업창이 나타나면 해당 Access Token 값을 향후 IdP 측 토큰 정보에 기입할 수 있도록 복사해둡니다. 해당 토큰 값은 다시 조회가 어려우므로 해당 창을 닫지 않고 바로 원장이 될 Identity Provider로 넘어가시는 것을 권장드립니다. (Okta-QueryPie Provisioning 연동 스텝 참고)혹시라도 토큰 값을 분실한 경우, 기존의 토큰을 삭제하고 다시 4번 단계를 통해 새로 발급받으시기 바랍니다.
  1. 해당 토큰 값은 다시 조회가 어려우므로 해당 창을 닫지 않고 바로 원장이 될 Identity Provider로 넘어가시는 것을 권장드립니다. (Okta-QueryPie Provisioning 연동 스텝 참고)
...
4. 새로 생성된 토큰은 발급일 yyyy-MM-dd 기준으로 1년까지만 유효합니다.만료일이 도래하면 토큰은 Expired 처리되어 SCIM API 이용이 불가합니다. 관리자는 만료일이 도달하기 이전에 신규 토큰을 발급하여 기존의 IdP에 설정된 토큰을 변경해주어야 합니다.
