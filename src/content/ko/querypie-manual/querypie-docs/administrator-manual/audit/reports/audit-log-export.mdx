...
[li]
Alert Logs [JSON] **[10.3.0]**  

[li]
Restricted Data Access Logs [JSON] **[10.3.0]**  

[li]
Masked Data Access Logs [JSON] **[10.3.0]**  

[li]
Sensitive Data Access Logs [JSON] **[10.3.0]**  

[li]
Web Access History [CSV] **[11.0.0]** 

[li]
Web Event Audit [CSV] **[11.0.0]** 

[li]
Web App Role History [CSV] **[11.0.0]** 

[li]
JIT Access Control Logs [CSV] **[11.0.0]** 

[li]
Workflow Unmasking Request Logs [JSON] **[11.0.0]**  

[li]
Workflow Restricted Data Access Request [JSON] **[11.0.0]**  

[li]
Workflow DB Policy Exception Request Logs [JSON] **[11.0.0]**  
...
3. Status 에서는 태스크의 진행 상태를 확인할 수 있습니다.Processing : 감사 로그 추출이 진행 중입니다.Completed : 감사 로그 추출이 완료되었으며, 파일 다운로드가 가능합니다. (추출 완료 후 30일이 경과하였다면, 파일이 만료되어 다운로드가 불가능합니다.)Failed : 감사 로그 추출이 실패하였습니다. QueryPie Customer Support 를 통해 문의 바랍니다.
...
2. Log Type : 추출 대상 로그의 종류를 선택합니다. Query Audit 등 추출하기 원하는 로그 타입을 선택합니다.로그 종류 선택 후 See Log Template and Description을 클릭하시면 각 로그의 key 등 상세 정보를 조회할 수 있습니다.
...
4. For Text Exceeding 32,000 Characters : CSV 파일의 경우 32,000자를 초과하는 글자를 잘라낼지 여부를 선택할 수 있습니다.Trim Overflowing Text - 32,000자를 초과하는 글자를 잘라냅니다. Excel로 파일을 직접 열어야 하는 경우에 셀 당 최대 글자수 초과로 표가 깨지는 현상을 방지합니다.No Action - 32,000자를 초과하는 글자를 그대로 포함시킵니다.
...
6. To : 추출 대상 마지막 날짜를 지정합니다.
7. Filter Expression : 필터 표현식을 지정합니다. 조건 입력 방법 및 예시는 하단의 ‘필터 표현식’을 참고해주시기 바랍니다.
  1. 조건 입력 방법 및 예시는 하단의 ‘필터 표현식’을 참고해주시기 바랍니다.
8. Generate Preview : 미리보기를 생성합니다. 미리보기는 필수 단계입니다.미리보기 결과를 확인해야 다음 단계인 ‘Create’가 가능합니다.
  1. 미리보기는 필수 단계입니다.
...
- String Type 
지원하는 표현식 : `== (equals)` , `!= (not equals)` , `contains`  예 : `x == 'abc'` , `x != 'abc'` , `contains(x, 'ab')` 
- Boolean Type 
