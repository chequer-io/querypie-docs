...
<p align="center">
<div>[image-20250629-174842.png]()</div>
...
3. 테이블 최측 상단의 검색란을 통해 아래와 조건으로 검색이 가능합니다.a. User Name : 사용자 이름b. User Email : 사용자 이메일c. Web App Name : 웹 애플리케이션 이름
4. 검색 필드 우측 필터 버튼을 클릭하여 AND 조건으로 아래와 필터링이 가능합니다.Event : 이벤트 유형 (JIT Access Granted/JIT Access Revoked)Action At : 액션 발생 일시 범위
  1. Event : 이벤트 유형 (JIT Access Granted/JIT Access Revoked)
  2. Action At : 액션 발생 일시 범위
5. 테이블 우측 상단의 새로고침 버튼을 통해 로그 목록을 최신화할 수 있습니다.
6. 테이블에서 아래와 같은 정보를 제공합니다:No : 순번Action At : 액션 발생 일시Event : 이벤트 유형 (JIT Access Granted, JIT Access Revoked)User Type : 사용자 유형 (USER)Name : 사용자명Email : 사용자 이메일Web App : 웹 애플리케이션명Expiration Date : JIT 접근 권한 만료 일시Action By : 액션을 수행한 관리자명
  1. No : 순번
