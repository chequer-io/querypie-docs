...
1. AWS CLI 설치 및 구성
  ```bash
brew install awscli
aws configure  # Access Key, Secret Key 필요
  ```
2. kubectl 설치
  ```bash
brew install kubectl
  ```
3. Helm 설치 (버전 3.10.0 이상 필수)
  ```bash
brew install helm
  ```
4. 선택적 도구 설치 (추천)
  ```bash
brew install kubectx  # 컨텍스트 전환 도구
brew install fzf      # 명령줄 퍼지 파인더
  ```
...
1. 시작하기 전에 QueryPie를 설치를 위해 namespaces를 만듭니다.
```
...
1. MySQL StatefulSet과 Service를 생성합니다.
```bash
kubectl apply -f mysql.yml
```

1. MySQL Pod가 정상적으로 실행될 때까지 기다립니다.
```bash
...
1. Redis Deployment와 Service를 생성합니다.
```bash
kubectl apply -f redis.yml
```

1. Redis Pod가 정상적으로 실행될 때까지 기다립니다.
```bash
...
1. 아래 내용으로 `querypie.env` 파일을 생성합니다.
```ini
...
1. 아래 내용으로 `values.yml` 파일을 생성합니다.
```yaml
...
1. 라이선스를 등록합니다. (라이선스 파일를 준비해 둡니다.)
```bash
...
kubectl logs <pod-name> -n querypie --previous  # 재시작된 경우 이전 로그 확인
```

주요 문제 상태 및 확인사항:

* `ImagePullBackOff`: Harbor 레지스트리 인증 정보 확인
