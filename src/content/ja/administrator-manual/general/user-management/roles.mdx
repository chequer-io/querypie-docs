...
この文書では管理者役割の構成、ポリシーの種類および管理者役割管理方法を案内します。
...
*  **管理者役割** （Role） : ユーザーに付与される単位として、ポリシー（Policy）の集合です。
    * 基本提供役割はCreated by Systemで表記されています。
        * Owner役割はQueryPieシステム全体に対する権限を持ち変更および削除が制限されます。
...
もう使用しない管理者役割を削除する二つの経路を提供します。

<Callout type="important">
管理者役割が削除されると既存に該当役割を割り当てられたユーザーたちからも即座に該当役割が回収されます。管理者役割削除はキャンセルが不可能なので慎重に実行してください。
<figure data-layout="center" data-align="center">
...
Rolesリストで削除しようとする役割をチェックボックスで選択した後、テーブルヘッダーに表示される`Delete`ボタンをクリックします。確認モーダルで削除確認文句「DELETE」を正しく入力し`Delete`ボタンをクリックすると管理者役割削除が完了します。
...
Rolesリストで、削除しようとする管理者役割詳細ページに進入します。`Delete`ボタンをクリックして削除確認モーダルを開いた後、削除確認文句「DELETE」を正しく入力し`Delete`ボタンをクリックすると管理者役割削除が完了します。
