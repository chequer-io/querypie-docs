...
PermissionまたはRoleでアクセス権限が付与されたサーバーに対してServer Privilegeを付与して一時的に特定のコマンド使用を許可できます。既存に既にServer Privilegeを付与した対象にServer Privilegeを再び付与する場合、既存のServer Privilegeを回収し新しく再び付与します。
