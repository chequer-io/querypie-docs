...
このドキュメントでは `10.2.7` 以下のバージョンに適用されたWACロールとポリシーについて案内します。
...
許可またはブロックポリシーを設定したいWebアプリケーションリソースを指定します。配下に `webApp`、`urlPaths` を持ちます。
...
2. Base URLを含めて、Webアプリケーションに存在する **すべて** のサブパスに対して許可または拒否する方法は以下の通りです。
    * urlPaths行を記述しません。
...
2. `resources[].urlPaths` と連携して動作します。
    *  **Webアプリケーションリソースの対象urlPathが明示されていない場合** 
    ...
    *  **Webアプリケーションリソースの対象urlPathが明示された場合** 
    ...
    *  **Webアプリケーションリソースの対象urlPathが明示されたが範囲内でマッチングされるurlPathTagがない場合** 
...
2. ポリシー構成時、タグおよびユーザー属性、IPなどの条件を使用します。 
    1. 制御対象WebアプリケーションのurlPathsを全体で指定しても、タグ条件付与時、Path Managementに登録されたパスを対象にのみポリシーが設定されます。
    2. 必要時ユーザー属性やIPなど、Webアプリケーションアクセスを許可するユーザーの範囲を指定できます。
