...
許可またはブロックポリシーを設定したいWebアプリケーションリソースを指定します。
...
Webアプリケーションのサブパスレベルでアクセス制御ポリシーを運用するためには、`webApp` 配下に `urlPaths` または `urlPathTags` を入力できます。
1つのポリシー内で、1つのWebアプリケーションに対して `urlPaths` と `urlPathTags` を同時に指定することは  **不可能** です。
...
WebアプリケーションにPathを登録し、各Pathに適切なタグを付与した後、ポリシーでタグベースにアクセスを制御することを推奨します。
